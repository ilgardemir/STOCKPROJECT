...
const I = {
  ...
  news:'<svg class="sec-icon" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2" stroke-linecap="round" stroke-linejoin="round"><path d="M4 22h16a2 2 0 0 0 2-2V4a2 2 0 0 0-2-2H8a2 2 0 0 0-2 2v16a2 2 0 0 1-2 2 2 2 0 0 1-2-2v-9a2 2 0 0 1 2-2h2"/><path d="M18 14h-8M15 18h-5M10 6h8v4h-8z"/></svg>'
};
...
function renderAll(d) {
  ...
  if (news.length) {
    const items = news.map(n => `<a class="news-item" href="${esc(n.url || "#")}" target="_blank" rel="noopener noreferrer">
      <div class="news-meta">${esc(n.date || "")}${n.source ? ` · ${esc(n.source)}` : ""}</div>
      <div class="news-title">${esc(n.title)}</div></a>`).join("");
    html += card("news", I.news, "Recent News", `<div class="news-list">${items}</div>`, { count: news.length, open: false });
  }
